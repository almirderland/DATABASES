...
WHERE department.dept_name = course.dept_name GROUP BY department.building HAVING count(1)=(SELECT MAX(second.number) FROM (SELECT count(1) as number FROM department,course WHERE department.dept_name = course.dept_name group by department.building) as second);
...
WHERE department.dept_name NOT IN (SELECT DISTINCT department.dept_name FROM department,student,takes WHERE department.dept_name=student.dept_name and student.id=takes.id and (takes.grade='F' or takes.grade='C'));

-- d
SELECT DISTINCT * FROM instructor
WHERE instructor.id NOT IN (SELECT DISTINCT instructor.id FROM instructor,teaches,takes WHERE instructor.id=teaches.id and teaches.course_id=takes.course_id and takes.grade='A');
